...
interface PsiquiatriaEArtesPostProps {
  post: {
    slug: string;
    title: string;
    subtitle: string;
...
export default function NeurocienciaPost({ post, psiquiatriaEArtesPosts }: PsiquiatriaEArtesPostProps) {
    ...
    <>
      ...
      <main className={styles.contentContainer}>
        ...
        <nav className={styles.navContent}>
          ...
          {psiquiatriaEArtesPosts.map(item => {
            if (post.slug !== item.slug) {
              return (
                <main key={item.id} style={{ backgroundImage: `url(${item.poster})` }}>
                  <div className={styles.navTextContent}>
                    <h1>{item.title}</h1>
                  </div>
                  <Link href={`/psiquiatriaEArtes/${item.slug}`}>
                    <a />
                  </Link>
                </main>
              )
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  ...
  const response = await prismic.getByUID('psiquiatriaeartesposts', String(slug), {
  ...
  const responsePsiquiatriaEArtesContent = await prismic.query(
    Prismic.predicates.at('document.type', 'psiquiatriaeartesposts')
  );

  const psiquiatriaEArtesPosts = responsePsiquiatriaEArtesContent.results.map(content => {
    return {
      id: content.id,
      slug: content.uid,
