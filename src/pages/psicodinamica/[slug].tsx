...
interface PsicodinamicaPostProps {
  post: {
    slug: string;
    title: string;
    subtitle: string;
...
export default function PsicodinamicaPost({ post, psicodinamicaPosts }: PsicodinamicaPostProps) {
            ...
            if (post.slug !== item.slug) {
              return (
                <main key={item.id} style={{ backgroundImage: `url(${item.banner})` }}>
                  ...
                  <Link href={`/psicodinamica/${item.slug}`}>
                    <a />
                  </Link>
                </main>
              )
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  ...
  const response = await prismic.getByUID('psicodinamicaposts', String(slug), {
  ...
  const psicodinamicaPosts = responseNeurocienciaContent.results.map(content => {
    return {
      id: content.id,
      slug: content.uid,
