...
interface NeurocienciaPostProps {
  post: {
    slug: string;
    title: string;
    subtitle: string;
...
export default function NeurocienciaPost({ post, neurocienciaPosts }: NeurocienciaPostProps) {
            ...
            if (post.slug !== item.slug) {
              return (
                <main key={item.id} style={{ backgroundImage: `url(${item.banner})` }}>
                  ...
                  <Link href={`/neurociencia/${item.slug}`}>
                    <a />
                  </Link>
                </main>
              )
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  ...
  const response = await prismic.getByUID('neurocienciaposts', String(slug), {
