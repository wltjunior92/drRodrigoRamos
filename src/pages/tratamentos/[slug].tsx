...
export default function NeurocienciaPost({ post, tratamentos }: PostProps) {
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  ...
  const response = await prismic.getByUID('tratamentos', String(slug), {
  ...
  const responseTratamentoContent = await prismic.query(
    Prismic.predicates.at('document.type', 'tratamentos')
  );

  const tratamentos = responseTratamentoContent.results.map(content => {
    return {
      id: content.id,
      slug: content.uid,
